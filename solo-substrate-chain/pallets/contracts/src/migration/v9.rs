...
impl<T: Config> MigrationStep for Migration<T> {
	...
	fn max_step_weight() -> Weight {
		T::ContractWeightInfo::v9_migration_step(T::MaxCodeLen::get())
	}
	...
	fn step(&mut self) -> (IsFinished, Weight) {
		...
		if let Some((key, old)) = iter.next() {
			...
			(IsFinished::No, T::ContractWeightInfo::v9_migration_step(len))
		} else {
			log::debug!(target: LOG_TARGET, "No more contracts code to migrate");
			(IsFinished::Yes, T::ContractWeightInfo::v9_migration_step(0))
		}
	}
