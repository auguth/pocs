	...
	<BalanceOf<T> as HasCompact>::Type: Clone + Eq + PartialEq + Debug + TypeInfo + Encode,
{
	...
	fn set_balance(&self, balance: BalanceOf<T>) {
		T::ContractCurrency::make_free_balance_be(&self.account_id, balance);
	}
...
benchmarks! {
	...
	verify {
		...
		let code_deposit = T::ContractCurrency::reserved_balance(&caller);
		assert_eq!(
			T::ContractCurrency::free_balance(&caller),
	...
	verify {
		...
		assert_eq!(
			T::ContractCurrency::free_balance(&instance.caller),
			caller_funding::<T>() - instance.value - value - deposit - Pallet::<T>::min_balance(),
		);
		// contract should have received the value
		assert_eq!(T::ContractCurrency::free_balance(&instance.account_id), before + value);
	...
	verify {
		// uploading the code reserves some balance in the callers account
		assert!(T::ContractCurrency::reserved_balance(&caller) > 0u32.into());
	...
	remove_code {
		...
		assert_eq!(uploaded.deposit, T::ContractCurrency::reserved_balance(&caller));
		assert!(<Contract<T>>::code_exists(&hash));
	}: _(origin, hash)
	verify {
		// removing the code should have unreserved the deposit
		assert_eq!(T::ContractCurrency::reserved_balance(&caller), 0u32.into());
	...
	verify {
		if r > 0 {
			assert_eq!(<T::ContractCurrency as Currency<_>>::total_balance(&instance.account_id), 0u32.into());
			assert_eq!(<T::ContractCurrency as Currency<_>>::total_balance(&deposit_account), 0u32.into());
			assert_eq!(<T::ContractCurrency as Currency<_>>::total_balance(&beneficiary), Pallet::<T>::min_balance() * 2u32.into());
		}
	}
	...
	verify {
		for account in &accounts {
			assert_eq!(<T::ContractCurrency as Currency<_>>::total_balance(account), value);
		}
	}
