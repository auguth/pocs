...
impl<T: frame_system::Config> AccountStakeInfo<T> {
    ...
    pub fn update(owner: &T::AccountId, delegate_to: &T::AccountId)-> Self{
        // Get the current block number.
        let current_block_number = <frame_system::Pallet<T>>::block_number();
        // Create and return a new AccountStakeInfo instance.
        let new_owner = owner.clone();
        let new_delegate_to = delegate_to.clone();
        Self {
            owner : new_owner,
            delegate_to : new_delegate_to,
            delegate_at:current_block_number,
        }
    }
...
impl<T: frame_system::Config> ContractScarcityInfo<T>{
    ...
    pub fn reset(current_reputation: &u64)-> Self{
        // Get the current block number.
        let current_block_number = <frame_system::Pallet<T>>::block_number();
        Self{
    ...
    pub fn update(
        ...
        if current_block_height > *old_block_height{
            ...
            Self{
                ...
                recent_blockheight: current_block_height,
            ...
            Self{
                reputation: *current_reputation,
                recent_blockheight: *old_block_height,
