...
# Function to process and save Parquet data within a specific range
def process_and_save(parquet_file, output_folder, range_data):
    try:
        print(f"Processing: {parquet_file}")
        
		# Read the Parquet file into a PyArrow table and convert it to a Pandas DataFrame
		...
		# Filter rows within the specified block range
        ...
        if not filtered_df.empty:  # Only create folder and file if data exists in the range
			...
			# Write the filtered data back to a Parquet file
...
# Function to process a single Parquet file with multiple ranges
...
if __name__ == "__main__":
    ...
    ranges = [
		# (Range Number, Start Block, End Block)
